...
button.MouseButton1Down:connect(function()
	...
	local function createTracer(plr)
		if plr == LocalPlayer or tracers[plr] then return end
	...
	local function updateESP()
		RunService.RenderStepped:Connect(function()
			...
			local fromScreen, onScreen1 = Camera:WorldToViewportPoint(myHRP.Position)

			for plr, tracer in pairs(tracers) do
				local char = plr.Character
				local hrp = char and char:FindFirstChild("HumanoidRootPart")
				local humanoid = char and char:FindFirstChildOfClass("Humanoid")

				if hrp and humanoid and humanoid.Health > 0 then
					local toScreen, onScreen2 = Camera:WorldToViewportPoint(hrp.Position)
...
Copyrighted (C) Fenrier 2013
	...
	spawn(function()
		...
		local currentPage = 1
		local pages = script.Parent.Pages:GetChildren()
		local totalPages = #pages

		local right = script.Parent:WaitForChild("RightButton")
		local left = script.Parent:WaitForChild("LeftButton")

		local page1 = script.Parent.Pages:FindFirstChild("Page1")
		local page5 = script.Parent.Pages:FindFirstChild("Page5")

		local function updatePage(newPage)
			if script.Parent.Pages:FindFirstChild("Page"..newPage) then
				currentPage = newPage
				for _, page in ipairs(pages) do
					page.Visible = false
				end
				script.Parent.Pages["Page"..newPage].Visible = true
			end
		end

		local function flipPage(direction)
			local targetPage = currentPage + direction
		...
		if page1 and page1:FindFirstChild("addonl") then
		...
		if page5 and page5:FindFirstChild("addonr") then
			page5.addonr.MouseButton1Down:Connect(function()
				currentPage = 0
				flipPage(1)
			end)
		end
